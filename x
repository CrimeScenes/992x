...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = getgenv().forbidden.PointAssist.Reach_Visibility  
            Circle.Radius = getgenv().forbidden.PointAssist.Reach  
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().Target.Keybind:lower() then
        if getgenv().forbidden.PointAssist.Enabled then  -- Updated to new structure
            if IsTargeting then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if getgenv().forbidden.PointAssist.Align then  -- Update this to the correct field in the new structure
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * getgenv().forbidden.PointAssist.VelocityCompensation)  -- Updated to new structure
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * getgenv().forbidden.PointAssist.VelocityCompensation)  -- Updated to new structure
            end

            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if getgenv().forbidden.PointAssist.StabilizationEnabled then  -- Updated to new structure
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, getgenv().forbidden.PointAssist.Stabilization)  -- Updated to new structure
                else
